...
namespace TeamApp.Infrastructure.Persistence.Services
{
    ...
    {
        public IpCheckHandler(IHttpContextAccessor httpContextAccessor, IOptions<JWTSettings> jwtSettings)
        {
        ...
        {
            Claim ipClaim = context.User.FindFirst(claim => claim.Type == "ip");
            ...
            if (accessToken == null && !requirement.IpClaimRequired)
            {
                return Task.CompletedTask;
            }
            if (GetPrincipalFromExpiredToken(accessToken).Claims.ToList()[4].Value == IpHelper.GetIpAddress())
            {
